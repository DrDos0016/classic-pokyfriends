...
def index(request):
    ...
    # Wider options for pokemon names
    user_starts_with = None
    if version >= 2 and starts_with and starts_with.title() in POKEMON_NAMES:
        # Use a random pokemon name as the starting word, and later replace it with the specified one
        user_starts_with = starts_with
        starts_with = random.choice(POKEMON_NAMES)
    ...
    # Wider options for pokemon names part 2
    if user_starts_with and version >= 2:
        post = user_starts_with + post[post.find(" "):]
...
def to_json(request):
    ...
    starts_with = request.GET.get("starts_with", "")
    seed        = str(request.GET.get("seed", default_seed))
    version     = int(request.GET.get("v", 2)) # Default to v2
    random.seed(seed)
    ...
    data["link"] = "https://pokyfriends.com/pokedex-ebooks/?size="+str(length)+"&ambiguous="+str(int(ambiguous))+"&starts_with="+user_starts_with+"&seed="+seed
