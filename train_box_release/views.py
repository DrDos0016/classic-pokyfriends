...
def custom(request):
    ...
    return render(request, "tbr_custom.html", data)

def index(request, poke1=None, poke2=None, poke3=None):
    data = {"title":"Train, Box, Release"}
    data["protocol"] = "http://"
    ...
    if (poke1 == None or poke2 == None or poke3 == None) or (poke1 == poke2) or (poke2 == poke3) or (poke1 == poke3):
        ...
        pokemon.append({"number":poke1, "name":NUM_TO_NAME[int(poke1)]})
        pokemon.append({"number":poke2, "name":NUM_TO_NAME[int(poke2)]})
        pokemon.append({"number":poke3, "name":NUM_TO_NAME[int(poke3)]})
            
    data["pokemon"] = pokemon
    
    return render(request, "tbr_index.html", data)
    
def results(request, poke1, poke2, poke3):
    data = {"title":"Train, Box, Release - Results"}
    data["protocol"] = "http://"
    ...
    pokemon = []
    pokemon.append({"number":poke1, "name":NUM_TO_NAME[int(poke1)], "choice":request.GET.get("c1"), "trio_stats":{"train":0, "box":0, "release":0, "total":0}, "overall_stats":{"train":0, "box":0, "release":0, "total":0}})
    ...
    for stat in data["trio_stats"]:
        if stat["pokemon"] == int(poke1):
            pokemon[0]["trio_stats"][stat["option"]] = stat["count"]
        elif stat["pokemon"] == int(poke2):
            pokemon[1]["trio_stats"][stat["option"]] = stat["count"]
        elif stat["pokemon"] == int(poke3):
            pokemon[2]["trio_stats"][stat["option"]] = stat["count"]
            
    pokemon[0]["trio_stats"]["total"] = pokemon[0]["trio_stats"]["train"] + pokemon[0]["trio_stats"]["box"] + pokemon[0]["trio_stats"]["release"]
    pokemon[1]["trio_stats"]["total"] = pokemon[1]["trio_stats"]["train"] + pokemon[1]["trio_stats"]["box"] + pokemon[1]["trio_stats"]["release"]
    pokemon[2]["trio_stats"]["total"] = pokemon[2]["trio_stats"]["train"] + pokemon[2]["trio_stats"]["box"] + pokemon[2]["trio_stats"]["release"]
    
    # Get overall stats
    data["overall_stats"] = Answer.objects.filter(pokemon__in=[poke1,poke2,poke3]).values("option", "pokemon").annotate(count=Count("pokemon"))
    for stat in data["overall_stats"]:
        if stat["pokemon"] == int(poke1):
            pokemon[0]["overall_stats"][stat["option"]] = stat["count"]
        elif stat["pokemon"] == int(poke2):
            pokemon[1]["overall_stats"][stat["option"]] = stat["count"]
        elif stat["pokemon"] == int(poke3):
            pokemon[2]["overall_stats"][stat["option"]] = stat["count"]
            
    pokemon[0]["overall_stats"]["total"] = pokemon[0]["overall_stats"]["train"] + pokemon[0]["overall_stats"]["box"] + pokemon[0]["overall_stats"]["release"]
...
def submit(request):    
    try:
        trio = [request.POST.get("poke1"), request.POST.get("poke2"), request.POST.get("poke3")]
        trio.sort()
        trio = str(trio[0])+"/"+str(trio[1])+"/"+str(trio[2])
        answer = Answer(
            pokemon=request.POST.get("poke1"),
            option=request.POST.get("choice1"),
            trio=trio,
            ip=request.META["REMOTE_ADDR"]
        )
        
        answer2 = Answer(
            pokemon=request.POST.get("poke2"),
            option=request.POST.get("choice2"),
            trio=trio,
            ip=request.META["REMOTE_ADDR"]
        )
        
        answer3 = Answer(
            pokemon=request.POST.get("poke3"),
            option=request.POST.get("choice3"),
            trio=trio,
            ip=request.META["REMOTE_ADDR"]
        )
